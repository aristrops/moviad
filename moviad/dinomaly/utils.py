import torch
from moviad.dinomaly.dataset import get_data_transforms
...
def f1_score_max(y_true, y_score):
    ...
    f1s = f1s[:-1]
    best_idx = f1s.argmax()
    return f1s[best_idx], thrs[best_idx]
...
def evaluation_batch(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    ...
    with torch.no_grad():
        for img, label, gt, img_path in dataloader:
            ...
            anomaly_map, _ = cal_anomaly_maps(en, de, img_size)
            ...
            if resize_mask is not None:
                ...
                gt = F.interpolate(gt.float(), size=resize_mask, mode='nearest')
        ...
        f1_sp, _ = f1_score_max(gt_list_sp, pr_list_sp)
        f1_px, _ = f1_score_max(gt_list_px, pr_list_px)
...
def evaluation_batch_loco(model, dataloader, device, _class_=None, max_ratio=0):
    ...
    with torch.no_grad():
        for img, label, gt, path, defect_type, size in dataloader:
            img = img.to(device)
