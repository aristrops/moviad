...
class MVTecDataset(Dataset):
    def __init__(self, image_list, transform=transforms.ToTensor()):
        self.image_list = image_list
        self.transform = transform
        self.dataset = self.load_dataset()

    def load_dataset(self):
        return [self.transform(Image.open(p).convert("RGB")) for p in self.image_list]

    def __len__(self):
        return len(self.dataset)

    def __getitem__(self, idx):
        return self.image_list[idx], self.dataset[idx]
...
class StfpmBackbone(nn.Module):
    def __init__(
        ...
        if model_name in TORCH_BACKBONES:
            # For older torchvision, use pretrained parameter instead of weights
            pretrained = weights is not None and weights != "None"
