...
class CustomFeatureCompressor():
    ...
    def get_and_compress_features(self, dataset, feature_extractor):
        ...
        for image in dataset:
            ...
            features = self.apply_feature_compression(features)
    ...
    def apply_feature_compression(self, features):
        ...
        if "random_sampling" in self.feature_compression_method:
            ...
            if "webp_for_features" in self.feature_compression_method:
                features = self.compress_features_with_codec(features, self.quality)

        #random sampling of ViT features
        if "spatial_sampling" in self.feature_compression_method:
            features = self.spatial_subsample_tokens(features, self.compression_ratio)
    ...
    def sample_channels(self, features, compression_ratio):
        ...
        for f in features:
            ...
            sampled_idxs = self.sampled_indices_cache[key]
    ...
    # for ViT features
    def spatial_subsample_tokens(self, features, compression_ratio):

        if compression_ratio == 1:
            return features

        stride = max(1, int(1 // compression_ratio))
        cls_offset = 1 + self.num_register_tokens

        subsampled = []
        for f in features:
            B, N, C = f.shape
            prefix_tokens = f[:, :cls_offset, :]
            patch_tokens = f[:, cls_offset:, :]

            T = patch_tokens.shape[1]
            side = int(math.sqrt(T))
            if side * side != T:
                raise ValueError(f"Patch token count {T} is not a perfect square; cannot form a spatial grid.")

            grid = patch_tokens.reshape(B, side, side, C)
            sampled_grid = grid[:, ::stride, ::stride, :]

            new_side = sampled_grid.shape[1]
            if new_side == 0:
                raise ValueError(f"Stride {stride} is too large for grid side {side}; nothing left to sample.")

            sampled_patches = sampled_grid.reshape(B, new_side * new_side, C)
            subsampled.append(torch.cat([prefix_tokens, sampled_patches], dim=1))

        return subsampled

    #----------FEATURE COMPRESSION WITH WEBP--------------
    ...
    def compress_features_with_codec(self, features, quality):
        ...
        for f in features:
            ...
            compressed_image = self.apply_image_compression(img, quality, compress_features=True)
    ...
    def collect_feature_vectors(self, dataset, feature_extractor):
        ...
        for image in dataset:
            with torch.no_grad():
                ...
                if "spatial_sampling" in self.feature_compression_method:
                    features = self.spatial_subsample_tokens(features, compression_ratio=self.compression_ratio)

                self.original_shapes = [f.shape[1:] for f in features]

                flatten_features = torch.cat([f.flatten() for f in features])
                feature_vectors.append(flatten_features.detach().cpu())
    ...
    def decompress_features(self, compressed_features):
        ...
        for shape_tuple in self.original_shapes:
            ...
            decoded_features.append(segment.reshape(*shape_tuple))
            offset += num_elements
        return decoded_features

    # --------AUTOENCODERS--------

    def train_autoencoders(self, train_dataloader, feature_extractor, optimizers, device, epochs: int = 10,
                           noise_std: int | None = None) -> float:
    ...
    def apply_image_compression(self, image, quality, compress_features = False):
